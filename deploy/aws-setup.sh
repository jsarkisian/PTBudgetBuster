...
if command -v aws &>/dev/null; then
    if aws bedrock list-foundation-models --region "$AWS_REGION" --query 'modelSummaries[?modelId==`anthropic.claude-opus-4-20250514`].modelId' --output text 2>/dev/null | grep -q "anthropic"; then
